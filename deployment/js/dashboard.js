...
import { showToast, formatDate, formatFileSize } from './utils/validators.js';
...
class Dashboard {
    ...
    setupRealTimeListeners() {
        ...
        const requestsUnsubscribe = realTimeListener(
            ...
            (error) => {
                console.error('Error in swap requests listener:', error);
            }
        );
    ...
    updateStatsDisplay(stats) {
        ...
        document.getElementById('itemsShared').textContent = stats.itemsShared || 0;
        document.getElementById('co2Saved').textContent = `${stats.co2Saved || 0}kg`;
        document.getElementById('exchangesMade').textContent = stats.exchangesMade || 0;
        
        // Calculate growth metrics
        const pointsGrowth = stats.pointsThisMonth || 0;
        document.getElementById('pointsGrowth').textContent = `+${pointsGrowth}`;
        
        const newItemsThisMonth = stats.newItemsThisMonth || 0;
        document.getElementById('newItemsThisMonth').textContent = newItemsThisMonth;

        // Update level progress
        const level = Math.floor((stats.totalPoints || 0) / 100) + 1;
        const progressInLevel = (stats.totalPoints || 0) % 100;
        
        document.getElementById('userLevel').textContent = level;
        document.getElementById('levelProgress').textContent = `${progressInLevel}/100`;
        document.getElementById('levelProgressBar').style.width = `${progressInLevel}%`;
    }
    ...
    createItemCard(item) {
        const statusColor = item.status === 'available' ? 'bg-green-100 text-green-800' :
                           item.status === 'exchanged' ? 'bg-blue-100 text-blue-800' :
                           item.status === 'redeemed' ? 'bg-purple-100 text-purple-800' :
    ...
    async loadSwapRequests() {
        try {
            const requests = await getSwapRequests(this.currentUser.uid, 'received');
            this.swapRequests = requests.filter(req => req.status === 'pending');
            this.renderSwapRequests();
        } catch (error) {
