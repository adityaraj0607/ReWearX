...
// Firebase Configuration - using the new config
...
class AuthManager {
  ...
  async createUserDocument(user, additionalData = {}) {
    ...
    try {
      ...
      // Silently handle errors with user document creation
    }
  }
  ...
  updateUIForAuthState(user) {
    ...
    if (user) {
      ...
      userNameElements.forEach(el => {
        el.textContent = `${user.displayName || user.email || 'User'} (${user.uid.substring(0, 6)})`;
      });
